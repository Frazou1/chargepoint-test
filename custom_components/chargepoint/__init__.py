...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    """Load the saved entities (cookies-only)."""
    await monkeypatch.ensure_scraper(hass)
    monkeypatch.apply_scoped_patch()

    _LOGGER.info(
        "Version %s starting (cookies-only). Report issues: %s",
        VERSION,
        ISSUE_URL,
    )

    username = entry.data.get(CONF_USERNAME, "")
    # password non utilisé mais laissé pour compatibilité structurelle
    password = entry.data.get(CONF_PASSWORD, "")
    use_cookie_auth = bool(entry.data.get(CONF_COOKIE_AUTH, False))

    if not use_cookie_auth:
        _LOGGER.error("This build requires cookie-based auth. Re-add the integration with cookies.")
        raise ConfigEntryAuthFailed("cookies_required")

    await hass.async_add_executor_job(remove_session_token_from_disk, hass)

    try:
        # IMPORTANT: 2 arguments seulement → aucun token passé → pas de validation de format
        client: ChargePoint = await hass.async_add_executor_job(ChargePoint, username, password)
    except ChargePointBaseException as exc:
        # Erreurs lib génériques au setup → demander de re-coller des cookies
        _LOGGER.error("Failed to initialize ChargePoint client (cookies-only).")
        raise ConfigEntryAuthFailed("cookie_init_failed") from exc

    hass.data.setdefault(DOMAIN, {})

    async def async_update_data(is_retry: bool = False):
        """Fetch data from ChargePoint API using cookies only."""
        data = {
            ACCT_INFO: None,
        ...
        try:
            account: ChargePointAccount = await hass.async_add_executor_job(client.get_account)
            _LOGGER.debug("Account information: %s", account)
            data[ACCT_INFO] = account

            crg_status: Optional[UserChargingStatus] = await hass.async_add_executor_job(client.get_user_charging_status)
            _LOGGER.debug("User charging status: %s", crg_status)
            data[ACCT_CRG_STATUS] = crg_status

            if crg_status:
                crg_session: ChargingSession = await hass.async_add_executor_job(client.get_charging_session, crg_status.session_id)
                _LOGGER.debug("Charging session: %s", crg_session)
                data[ACCT_SESSION] = crg_session

            home_chargers: list = await hass.async_add_executor_job(client.get_home_chargers)
            for charger in home_chargers:
                hcrg_status: HomeChargerStatus = await hass.async_add_executor_job(client.get_home_charger_status, charger)
                hcrg_tech_info: HomeChargerTechnicalInfo = await hass.async_add_executor_job(
            ...
            # Cookies expirés/invalides → on demande à l’utilisateur d’en recoller
            _LOGGER.error("ChargePoint cookie session expired/invalid. Please re-import cookies in the config flow.")
    ...
    if poll_interval not in POLL_INTERVAL_OPTIONS.values():
        _LOGGER.warning("Invalid poll interval %s, using default %s", poll_interval, POLL_INTERVAL_DEFAULT)
...
class ChargePointChargerEntity(CoordinatorEntity):
    ...
    def __init__(self, client: ChargePoint, coordinator: DataUpdateCoordinator, charger_id: int):
        super().__init__(coordinator)
        self.client = client
        ...
        self._attr_device_info = DeviceInfo(
            ...
            name=(f"{self.manufacturer} Home Flex ({self.short_charger_model})" if "CPH" in self.short_charger_model else f"{self.manufacturer} {self.short_charger_model}"),
    ...
    def session(self) -> Optional[ChargingSession]:
        ...
        if not session:
            return
        _LOGGER.debug("Session in progress, checking if home charger (%s): %s", self.charger_id, session)
